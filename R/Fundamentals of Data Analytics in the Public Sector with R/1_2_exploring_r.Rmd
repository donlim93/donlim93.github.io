...
application_ratio <- sum(cleaned_df$APPLCNM) / (sum(cleaned_df$APPLCNW) + sum(cleaned_df$APPLCNM))
...
```{r}
question_one <- admissions_df[1337, "ENRLW"]
```

```{r}
question_two <- admissions_df[is.na(admissions_df$ADMSSNM) | is.na(admissions_df$ADMSSNW), ]
paste0(
       "We have ", nrow(missing_schools),
       " schools which have reported NA for both men and women."
)
...
```{r}
question_three <- mean(admissions_df[!is.na(admissions_df$ADMSSNM), c("ADMSSNM")])
```
